describe('Book CRUD Operations', () => {
  ...
  it('should add a new book', () => {
    ...
    cy.get('input[name="publishedYear"]').type(
      newBook.publishedYear.toString()
    );
    cy.contains('button', 'Add Book').click();

    cy.contains(
      'li',
      `${newBook.title} by ${newBook.author} (${newBook.publishedYear})`
    );
  });
  ...
  it('should edit an existing book', () => {
    ...
    cy.get('input[name="title"]')
      .clear()
      .type('To Kill a Mockingbird (Updated)');
